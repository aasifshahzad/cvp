...
import type { HeroSectionResponse } from "@/lib/types";
...
export function Hero({ data }: HeroProps = {}) {
  // Use data from backend if available, otherwise use default marketing copy
  const title = data?.title || "Pakistan's Homeopathic Doctors, Now Digital";
  const subtitle =
    data?.subtitle ||
    "Find a verified homeopathic doctor near you — or bring your practice online with your own digital clinic.";
  const credentials = data?.credentials || [
    { label: "", value: "Verified Doctors", order: 1 },
    { label: "", value: "Easy Online Booking", order: 2 },
    { label: "", value: "Natural & Holistic Care", order: 3 },
    { label: "", value: "Free to Join for Doctors", order: 4 },
  ];
  return (
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto pt-8">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-card p-6 border border-white/20 hover:bg-white/15 transition-all">
              ...
              <Link href="/doctors">
                <Button
                  variant="primary"
                  className="w-full bg-white text-primary-dark hover:bg-gray-100"
                >
                  Find a Doctor
                </Button>
              </Link>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-card p-6 border border-white/20 hover:bg-white/15 transition-all">
              ...
              <Link href="/register">
                <Button
                  variant="primary"
                  className="w-full bg-accent text-primary-dark hover:bg-accent/90"
                >
                  Join as a Doctor
                </Button>
              </Link>
