...
| Command                                 | What it does                                                                                           |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `npm run dev` _(from root)_             | Starts both backend and frontend together using concurrently. Color-coded output in a single terminal. |
...
| `npm run generate-client` _(from root)_ | Runs API client generator from root level — no need to `cd` into the frontend folder.                  |
...
> **Tip:** Run `npm run generate-client` whenever backend routes or models change. Keep `.env` files in both `cvp_backend/` and `cvp_dashboard/` for correct local configuration.
